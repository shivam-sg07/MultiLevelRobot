...
var goal_threshold = 1.0  # or 0.5, depending on your environment's scale
...
func get_reward() -> float:
	...
	# Penalty for not making progress for N steps
	if n_steps % 50 == 0 and current_goal_position >= closest_goal_position:
		reward_delta -= 0.1

	# Penalty for obstacle collision (if implemented)
	#if robot.is_colliding_with_obstacle():
	#	reward_delta -= 0.1
