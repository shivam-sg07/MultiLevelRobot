...
eval_callback = EvalCallback(env, best_model_save_path="./logsEval/",log_path="./logsEval/", eval_freq=500,deterministic=True, render=False)
...
if args.resume_model_path is None:
    model = DQN(
    "MlpPolicy",  # or "CnnPolicy" if your obs is image-based
    env,
...
if args.inference:
    ...
    for i in range(args.timesteps):
        ...
        if done:
            print(f"Episode ended with reward: {episode_reward}")
            obs = env.reset()
            episode_reward = 0
else:
